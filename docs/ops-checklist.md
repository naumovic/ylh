...
- **Empty build-time env var ≠ unset.** `VITE_POSTHOG_HOST` was set **blank** on Render, and `analytics.ts` used `??` (which only falls back on null/undefined), so an empty host slipped through and posthog-js posted events to the **current origin** (`POST /e/` → 404 on our own server) instead of PostHog — Live events stayed empty with no error. Fixed by using `||` (commit `8c24cf3`). If PostHog shows nothing, check the request's target host in DevTools first: it must be `us.i.posthog.com/e/`, not our domain. Leaving `VITE_POSTHOG_HOST` blank is now harmless, but cleaner to delete it or set `https://us.i.posthog.com`.
